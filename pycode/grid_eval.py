...
class AtmospherePackage:
    ...
    def info(self):
        print(
            ...
            + "Solar time coverage: "
            + str(self.slts_covered)
            + "\n"
            + "Coverage percentage: "
            + str(self.slts_coverage_percentage)
            + "%\n"
            + "times: "
            + str(self.points[:, 7])
        )
...
class AltitudeSection:
    ...
    def sort_points_into_packages(self, d_lat: float, d_lon: float):
        ...
        self.d_lat = d_lat
        self.d_lon = d_lon
        number_of_latitude_sections = int(np.pi / d_lat)  # per altitudesection
        self.number_of_latitude_sections = number_of_latitude_sections
        number_of_longitude_sections = int(
            2 * np.pi / d_lon
        )  # per altitude section and latitude section
        ...
        for i in range(number_of_latitude_sections):
            ...
            for j in range(number_of_longitude_sections):
                ...
                points = self.points[
                    (self.points[:, 4] >= lat_min)
                    & (self.points[:, 4] < lat_max)
                    & (self.points[:, 5] >= lon_min)
                    & (self.points[:, 5] < lon_max)
                ]

                # print('  ', lon_min, lon_max, '---', points[:,7])

                atm_p = AtmospherePackage(
                    self.h_min, self.h_max, lat_min, lat_max, lon_min, lon_max, points
                )
                atm_p.index = (i, j)
    ...
    def calculate_coverage_matrix(self):
        ...
        if self.d_lat == 0 or self.d_lon == 0:
            raise ValueError(
                "Latitude and longitude increments must be set before calculating point density matrix.  \nRun sort_points_into_packages() method first."
            )

        # create matrix with number of points in each package
        data = np.zeros(
            (self.number_of_latitude_sections, self.number_of_longitude_sections)
        )
        for i in range(self.number_of_latitude_sections):
            for j in range(self.number_of_longitude_sections):
                data[i, j] = self.packages[
                    i * self.number_of_longitude_sections + j
                ].number_of_points

        self.coverage_matrix = data.astype(int)
        # count packages that have at least one point
        self.number_of_empty_packages = len(
            [p for p in self.packages if p.number_of_points == 0]
        )
    ...
    def plot_coverage(
        self, show_plot: bool = True, save_plot: bool = True, orbit_name: str = None
    ):

        deg_to_rad = np.pi / 180

        if self.coverage_matrix.all() is None:
        ...
        plt.imshow(data, cmap=cm, interpolation="nearest", vmin=1)

        # Add color bar
        plt.colorbar(extend="min", label="Number of measurements")

        # Label x-axis with longitude
        plt.xticks(
            np.arange(self.number_of_longitude_sections),
            [
                f"{int(-180 + (i * self.d_lon / deg_to_rad))} to {int(-180 + ((i+1) * self.d_lon / deg_to_rad))}"
                for i in range(self.number_of_longitude_sections)
            ],
            rotation=-90,
        )
        ...
        plt.yticks(
            np.arange(self.number_of_latitude_sections),
            [
                f"{int(90 - (i * self.d_lat / deg_to_rad))} to {int(90 - ((i+1) * self.d_lat / deg_to_rad))} "
                for i in range(self.number_of_latitude_sections)
            ],
        )
        ...
        plt.title(
            f"Point density at altitude {self.h_min / 1000} to {self.h_max / 1000} km; \n {self.points.shape[0]} points."
        )

        # Add vertical line at 0 degree longitude (greenwich meridian)
        plt.axvline(
            x=(self.number_of_longitude_sections - 1) / 2, color="red", linestyle="--"
        )

        # Add horizontal line at equator
        plt.axhline(
            y=(self.number_of_latitude_sections - 1) / 2, color="red", linestyle="--"
        )

        plt.tight_layout()

        # safe plot in plots/"orbit_name"/"altitude_section"
        if orbit_name is not None and save_plot is True:
            path = os.path.join("plots", orbit_name)
            if not os.path.exists(path):
                os.makedirs(path)
            plt.savefig(
                os.path.join(
                    path, f"altitude_section_{self.h_min/1000}_{self.h_max/1000}.png"
                )
            )
...
class AtmosphereGrid:
    ...
    def __init__(self, h_low_bound, h_high_bound, d_h, d_lat, d_lon) -> None:
        ...
        self.number_of_packages = (
            self.number_of_altitude_sections
            * self.number_of_latitude_sections
            * self.number_of_longitude_sections
        )
        print("atmosphere grid created")

    def print_info(self):
        print(
            f"Atmosphere grid from {self.h_low_bound} to {self.h_high_bound}"
            " km altitude."
        )
...
class SatelliteOrbit:
    ...
    def generate_random_data(self, k: int):
        ...
        self.timestamps = np.linspace(
            0, 3600, k
        )  # k timestamps evenly spaced over 3600 hours
    ...
    def read_from_files(self, load_path: str, sample_rate: int = 0):
        ...
        original_sample_rate = len(self.timestamps) / (
        ...
        if sample_rate == 0:
            ...
            self.timestamps = self.timestamps[
                :: int(original_sample_rate / sample_rate)
            ]
            ...
            self.longitudes = self.longitudes[
                :: int(original_sample_rate / sample_rate)
            ]
    ...
    def calculate_local_time(self):
        ...
        # start at 0 hours and 0 degree longitude (greenwich meridian) is used here
        # (irrelevant for this project, since relative local time is used)
        deg_to_rad = np.pi / 180

        # Calculate solar local time for each longitude
        for i in range(len(self.timestamps)):
            slt = np.round(
                (self.timestamps[i] + (self.longitudes[i] / (15 * deg_to_rad))) % 24, 0
            )  # solar local time
            # print(slt)
            if slt == 24:
                slt = 0
    ...
    def calculate_fraction_below_altitude(self, threshold_altitude: float) -> float:
        """
        ...
        """
        total_points = len(self.altitudes)
        below_threshold_points = sum(
            altitude < threshold_altitude for altitude in self.altitudes
        )
        fraction_below_altitude = below_threshold_points / total_points
        return fraction_below_altitude
...
class GridCoverageAnalyzer:
    ...
    def analyze(self):
        ...
        for i in range(self.atm_grid.number_of_altitude_sections):
            ...
            print(
                f"Analyzing altitude section {i+1}"
                " of {self.atm_grid.number_of_altitude_sections}"
            )
            h_min = self.atm_grid.h_low_bound + i * self.atm_grid.d_h
            h_max = self.atm_grid.h_low_bound + (i + 1) * self.atm_grid.d_h
            points = self.points[
                (self.points[:, 6] >= h_min) & (self.points[:, 6] < h_max)
            ]
            altitude_sec = AltitudeSection(h_min, h_max, points)
            altitude_sec.sort_points_into_packages(
                self.atm_grid.d_lat, self.atm_grid.d_lon
            )
    ...
    def print_statistics(self):
        ...
        for i, altitude_section in enumerate(self.list_of_altitude_sections):
            ...
            print(
                ...
                f" {altitude_section.number_of_latitude_sections}"
            )
            print(
                f"  Number of longitude sections: {altitude_section.number_of_longitude_sections}"
            )
            print(f"  Coverage percentage: {altitude_section.coverage_percentage}%")
            print(
                f"  Average number of Solar local times covered per Package: {altitude_section.slt_count_avg} +/- {altitude_section.slt_count_std} hours"
            )
    ...
    def get_statistics_dataframe(self):
        ...
        for i, altitude_section in enumerate(self.list_of_altitude_sections):
            data["Altitude Section"].append(
                f" {altitude_section.h_min / 1000} to {altitude_section.h_max / 1000}km"
            )
            data["Number of Points"].append(altitude_section.points.shape[0])
            data["Number of Packages"].append(len(altitude_section.packages))
            data["Number of empty Packages"].append(
                altitude_section.number_of_empty_packages
            )
            data["Number of Latitude Sections"].append(
                altitude_section.number_of_latitude_sections
            )
            data["Number of Longitude Sections"].append(
                altitude_section.number_of_longitude_sections
            )
    ...
    def write_coverage_percentage_to_file(self, file_path: str):
        ...
        df_filtered = df[["Altitude Section", "Coverage Percentage"]].transpose(
            copy=True
        )
        df_filtered.to_csv(
            os.path.join(file_path, "coverage_percentage.csv"), index=False
        )
        print("Coverage percentage written to file")

    def plot_grid_coverage(self, onebyone: bool = False, show_plot: bool = False):
        if len(self.list_of_altitude_sections) == 0:
            raise ValueError(
                "No altitude sections to plot.  \nRun analyze() method first."
            )
        for i, altitude_section in enumerate(self.list_of_altitude_sections):

            print(
                f"Plotting altitude section {i+1} of {len(self.list_of_altitude_sections)}"
            )
            altitude_section.plot_coverage(
                show_plot=onebyone, orbit_name=self.orbit.name
            )
        if show_plot:
            plt.show()

    def plot_slt_coverage(self, save_plot: bool = True, show_plot: bool = False):
        ...
        self.d = np.flipud(
            self.d
        )  # flip matrix upside down to have lowest altitude at the bottom
        ...
        fig, ax = plt.subplots()
        im = ax.imshow(self.d, cmap=cmap, interpolation="nearest")
        ...
        ax.legend(
            handles=patches, loc="upper center", bbox_to_anchor=(0.5, -0.25), ncol=2
        )

        plt.yticks(
            np.arange(len(self.list_of_altitude_sections)),
            [
                f"{int(altitude_section.h_min / 1000)} to {int(altitude_section.h_max / 1000)} km"
                for altitude_section in reversed(self.list_of_altitude_sections)
            ],
        )
        ...
        # safe plot in plots/"orbit_name"
        if save_plot:
            path = os.path.join("plots", self.orbit.name)
            if not os.path.exists(path):
                os.makedirs(path)
            plt.savefig(os.path.join(path, "slt_coverage.png"))
